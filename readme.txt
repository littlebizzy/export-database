...
Donate link:
Tags: export, database, db, migrate, download, backup, back up, mysql, sql, zip
...
Quickly and easily export your WordPress database with a single click for the purposes of migration, testing, or backup (in the original SQL format).

== Description ==

Export Database LittleBizzy is a simple plugin that does one thing only: with a single click you are able to export your WordPress site's database exactly as it exists on the live site.
...
* dozens of code snippets across the web
* custom coding
...
1. Upload the plugin files to `/wp-content/plugins/export-database-littlebizzy`
...
Currently no, there is no settings page and not meant to be at this time.
...
= 1.0 =
* Initial release inspired by other plugins and our own research.