...
class EXPDBS_Core_Export {
	...
	private static function get_table_rows($table, &$migration) {
		...
		// Set order
		$orderby = ' ORDER BY '.(empty($primary_keys) ? self::bq($first_field) : implode(', ', self::bq($primary_keys)));
		...
		foreach ($rows as $row) {
			...
			if (empty($dump)) {
				...
				// Insert into section
				$dump = 'INSERT INTO '.self::bq($table).'( '.implode(', ', self::bq(array_keys($row))).' ) VALUES'."\n";
			...
			foreach ($row as $field => $value) {
				...
				// Bit data replacement (bug fixed)
	...
	private static function bq($value) {
		...
		// Populate (PHP 8 safe replacement for deprecated each())
