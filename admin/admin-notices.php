 ...
 */
final class EXPDBS_Admin_Suggestions {
	...
	public static function instance() {
		...
		if (!isset(self::$instance))
			self::$instance = new EXPDBS_Admin_Suggestions;
	...
	*/
	private function __construct() {
		...
		$timestamp = (int) get_option( 'expdbs_dismissed_on' );

		if ( empty( $timestamp ) || ( time() - $timestamp ) > (180 * 86400) ) {

			// Check AJAX submit
			if (defined('DOING_AJAX') && DOING_AJAX) {
				add_action( 'wp_ajax_expdbs_dismiss', array(&$this, 'dismiss'));
...
<script type="text/javascript">jQuery(function($) {	$(document).on('click', '.expdbs-dismiss .notice-dismiss', function() { $.post(ajaxurl, {'action':'expdbs_dismiss','nonce':$(this).parent().attr('data-nonce')}); }); });</script>
	<?php }
	...
	public function dismiss() {
		if (!empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'expdbs-dismiss')) {
			update_option('expdbs_dismissed_on', time(), true);
		}
	}
	...
	public function admin_notices() {
		...
		$plugin_data = get_plugin_data( EXPDBS_FILE );

		?><div class="expdbs-dismiss notice notice-success is-dismissible" data-nonce="<?php echo wp_create_nonce('expdbs-dismiss'); ?>">
